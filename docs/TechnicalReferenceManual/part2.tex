...
\begin{defaulttable}
  ...
  \begin{tabularx}{\textwidth}{l X c}
    ...
    \hspace{0.5em} $\llcorner$ \pymodule/ & \simulator 친화적 구조 및 중간 변환 & - \\
    \hspace{0.5em} $\llcorner$ launcher/ & Excel 기반 입력파일의 실행을 위한 web 기반 GUI & \ref{section:dirstructure-excellauncher} \\
    \addlinespace 
    ...
    \texttt{runEngine} & 엔진 실행 스크립트 & \ref{subsection:dirstructure-enginelauncher} \\
    \texttt{runExcelLauncher} & Excel 기반 입력파일의 실행을 위한 web 기반 GUI 실행 스크립트 & \ref{subsection:dirstructure-excellauncher} \\
...
\section{명령줄 인터페이스 (CLI) 사용법}
\label{subsection:dirstructure-enginelauncher}
...
\label{section:dirstructure-excellauncher}
...
\section{Python 모듈 직접 사용법}



\pymodule~모듈의 \texttt{api.py}는 시뮬레이션 엔진의 핵심 기능을 수행하는 4개의 주요 함수를 제공한다.

\subsection{\texttt{run\_grjson}: json구조 입력파일(\texttt{grm} 확장자) 실행}
...
\subsection{스프레드시트 입력파일 실행 (run\_grexcel)}
