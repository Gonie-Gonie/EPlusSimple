...
\begin{defaulttable}
  ...
  \begin{tabularx}{\textwidth}{l X c}
    ...
    \hspace{0.5em} $\llcorner$ \texttt{ExcelInterfaceUseGuide} & Excel 이용 입력파일 작성 안내 & \ref{subsection:dirstructure-documentationEIUG} \\
    \addlinespace
...
\subsubsection{json구조 입력파일(grm) 실행 (run\_grjson)}

표준 JSON 포맷(\texttt{.grm})으로 작성된 모델 파일을 입력받아 시뮬레이션을 수행한다. 엔진의 가장 기본이 되는 실행 함수이다.

\begin{codebox}{run\_grjson}
\small
\textbf{Definition:}
\begin{verbatim}
...
\begin{table}[htbp]
    ...
    \midrule
    \texttt{input\_filepath} & \texttt{str} & 입력 파일 경로 (\texttt{.grm} 확장자 권장, 생략 가능) \\
    \texttt{output\_filepath} & \texttt{str | None} & 결과 파일(.grr) 저장 경로. \newline \texttt{None}일 경우 입력 파일명에 \texttt{\_out}을 붙여 자동 생성 \\
    \texttt{save} & \texttt{bool} & \texttt{True}이면 결과를 파일로 저장하고 경로(\texttt{str}) 반환. \newline \texttt{False}이면 저장하지 않고 결과 딕셔너리(\texttt{dict}) 반환 \\
    \bottomrule
    \end{tabularx}
\end{table}
