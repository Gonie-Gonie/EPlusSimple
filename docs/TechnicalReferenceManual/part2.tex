...
본 시뮬레이션 엔진은 명령행 인터페이스(Command-Line Interface, CLI)를 통해 실행할 수 있으며, 배포 패키지에는 이를 위한 배치 스크립트\inlinecode{runEngine.bat}가 포함되어 있다.
이 스크립트는 사용자가 Python 환경을 직접 호출할 필요 없이 프로그램을 실행할 수 있도록 하는 실행 래퍼(wrapper) 역할을 한다.
스크립트는 내부적으로 프로젝트에 포함된 독립실행형 Python을 호출하여 epsimple 모듈을 실행한다.

\begin{codeblock}{CLI command signature}
runEngine <command> [OPTIONS]
\end{codeblock}
...
\subsection{데이터베이스 조회}

\begin{codeblock}{CLI subcommand signature: `DB'}
runEngine DB <datatype> --get <item_id>
\end{codeblock}
...
\section{입력파일 (grm)} \label{section:grm-explanation}
...
\section{출력파일 (grr)} \label{section:grr-explanation}
