...
주소 정보는 \epreb 에서 허가일자와 함께 고려되어 건물의 기후지역(중부1, 중부2, 남부 등)을 판별하는 데 활용되며, 이에 따라 법적 기준 열관류율 등의 기준값을 적용하는 데 사용된다.\footnote{이에 대한 상세한 기술은 \ref{subsubsection:ioref:surface} 참고}  또한, 해당 지역 정보를 기반으로 기상데이터(EPW)와의 연계에도 활용된다.\footnote{이에 대한 상세한 기술은 \ref{sec:address_weather_matching} 참고}
...
\begin{table}[ht]
  ...
  GR후 :준공연도별 약 30\% 성능 향상 (슬라이딩 창호) \\
  GR후*:준공연도별 약 50\% 성능 향상 시스템 (이외 창호)}
...
\paragraph{난방 공급 설비} 난방 공급 설비의 이름. (\ref{subsubsection:ioref:surface} 참고 @여기)

\paragraph{냉방 공급 설비} 냉방 공급 설비의 이름. (\ref{subsubsection:ioref:surface} 참고 @여기)

\paragraph{환기 설비} 환기시스템의 이름. (\ref{subsubsection:ioref:surface} 참고 @여기)
...
존과 존, 외부 (외기 또는 지면), 또는 단열선 사이의 면(벽체)이다. 하나의 실은 \texttt{zone}을 인접존으로 하는 \texttt{surface}를 포함하여, 1개 이상의 천장(\texttt{type} = ``ceiling''), 1개 이상의 바닥(\texttt{type} = ``floor'') 및 1개 이상의 벽체(\texttt{type} = ``wall'')를 필요로 한다.
