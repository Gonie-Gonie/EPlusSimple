  ...
  \begin{tabular}{clllcc}
    \toprule
     재료명(국문) & \makecell{열전도도 \\ $[W{\cdot}m^{-1}{\cdot} K^{-1}]$}  & \makecell{밀도 \\ $[kg/m^{3}]$} & \makecell {비열 \\ $[J/kg\cdot K]$} & 재료명 (영문) &  출처 \\ \midrule
      콘크리트 & 2.5 & 2400 & 880 & concrete & \cite{ashrae_f18} \\
      단열재 & 0.05 & 30 & 1500 & insulation & \cite{ashrae_f18} \\
      석고보드 & 0.18 & 800 & 1090 & gypsumboard & \cite{ashrae_f18} \\
...
EnergyPlus 실행 결과물 종류와 유형, 그리고 그 결과를 \simulator 에서 어떻게 활용하는지에 대하여 추후 기술 예정임.
...
\begin{align}
E_{\mathrm{area}} &= \frac{\sum_{f} E_f \, c_f}{A} \\
G_{\mathrm{area}} &= \frac{\sum_{f} E_f \, g_f}{A} \\
C_{\mathrm{area}} &= \frac{\sum_{f} E_f \, p_f}{A}
\end{align}
...
\subsection{단위면적} \label{subsec:floorarea_definition_for_EUI}
단위면적의 개념에 대하여 추후 기술 예정임.
...
\subsection{온실가스 배출량}
연료별 온실가스 배출량 환산 계수는 표 \ref{tbl:coeff_kWh_to_CO2}\와 같다.
...
\paragraph{가스 및 등유} IPCC에서 기술하고 있는 바에 대하여 추후 기술 예정임.
...
\paragraph{지역난방} 지역난방은 한국지역난방공사의 경영실적 공시에 판매단가를 명시하고 있다. 자세한 값은 추후 기술 예정임.