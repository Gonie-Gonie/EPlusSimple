...
\subsection{Simplified Geometry (SG) 방식과 Detailed Geometry (DG) 방식}
...
\subsection{SG 입력 데이터의 DG 변환 방법}

\subsection{SG 입력의 EnergyPlus 시뮬레이션}


\subsection{벽체, 지붕, 바닥 변환 방법}

각 면과, 창호, 문, 블라인드 등의 변환 방법에 대하여 추후 기술 예정임.
...
EnergyPlus 및 \simulator 의 구조체 정의에 대하여 추후 기술 예정임.
...
재료의 출처 및 이것이 \simulator 에 적용되는 방법에 대하여 추후 기술 예정임 (아래 표는 database로 제시하고 있는 재료들의 예시).
...
실내외 열전달기준에 따른 구조체 자동 생성 및 EnergyPlus에서 이 값들이 계산되는 방식 등에 대하여 추후 기술 예정임.
...
냉난방설비시스템의 이원화 구조와, 냉난방설비, 전열교환기 및 태양광시스템의 변환 방법 기본값 등에 대하여 추후 기술 예정임.
