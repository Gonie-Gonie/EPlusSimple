...
본 엔진에서 EnergyPlus를 호출하는 구문, 디렉토리 구조 등에 대하여 추후 기술 예정임.
...
입력변수 오류, idf 변환 오류, EP 시뮬레이션 오류로 분류하여, 추후 엔진의 반환값을 정리한 후 각 종류와 대응 방법에 대하여 기술 예정임.
...
\chapter{Python 모듈} \label{chapter:pymodule}
...
\section{\pymodule~구조별 상세}
\
\pymodule 의 submodule 구조 및 기능에 대하여 추후 기술 예정임.
...
데이터 변환이 실제로 python 코드로 어떻게 구현되는지에 대하여 추후 기술 예정임.
...
EnergyPlus의 실행이 실제로 python 코드로 어떻게 구현되는지에 대하여 추후 기술 예정임.
...
python에서 \pymodule\을 활용하는 예시 코드에 대하여 추후 기술 예정임.